...
@click.option(
    ...
    default=0.01,
    type=float,
    help="P-value cutoff for the two-pass Mahalanobis distance test if -mode is MD or Poisson test if -mode is FC. Regions with p-values below this cutoff are considered significant variable features. Default is 0.01."
)
@click.option(
    "-mode",
    default="MD",
    type=click.Choice(["MD","FC"],case_sensitive=False),
    help="Variable feature detection method, default is MD. MD is based on a two-pass Mahalanobis distance test and is the default option. Chose FC to perfome a traditional fold chage > 2 selection using the average cutoff detected from MD method and perform the Poisson test.",
)
def patrol(r, c, t, o, lc, lt, pcut,mode):
    ...
    rprint(f"[{o}] Step 2: Performing two-pass Mahalanobis distance test with pcut = {pcut} to get average cutoff")
    distances, p_values = twoPassesMDTest(md_data, pcut=pcut)
    
    # Step 5: Perform Poisson test 
    rprint(f"[{o}] Step 3: Performing the Poisson test to compare {lc} vs {lt}")
    ...
    rprint(f"[{o}] Step 5: Generating MA plot of detected differential features and save them")
    if mode == "MD":
    ...
    rprint(f"[{o}] Step 6: Generating aggregated plots of detected differential features")
    for k, vs in rs.items():
